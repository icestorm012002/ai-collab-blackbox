...
AI 必须将整理好的 JSON 工作记录写入一个非真实的临时文件中（例如 `temp_worklog.json`）。AI 绝不可以自己去碰或试图写入最终的 `.ai/<AI_ID>/worklog.md` 等真实存证文件。
...
> 流程规定：你绝不能自己碰真实的 `.ai` 日志文件！你必须先把这一轮工作内容（包含时间戳、属于你的 AI_ID、负责的任务名、最后的操作状态、一句话梳理、任务清算表、修改文件列表及其引用说明等）写入一个临时的非真实的 JSON 记录文件中。
> 然后，你必须调用专门的 Python 落盘脚本（并使用 --file 参数指明你的临时文件），由这个特定的程序帮你把记录严格写进 worklog.md、worklog.jsonl，并在全局时间轴 all_time.jsonl 里做汇总。程序一旦完成写入，会在停顿 1 秒后自动帮你删去这个临时的 JSON 文件。
